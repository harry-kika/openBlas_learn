//
// Created by weifeng on 2021/2/6.
//
...
int Transformer_SVD::load_param(int d_model_, int nhead_, int dim_feedforward_)
{
    d_model = d_model_;
    nhead = nhead_;
...
void Transformer_SVD::set(int d_model_, int nhead_, int dim_feedforward_, float* self_attn_q_weight_, float* self_attn_k_weight_,
                          ...
                          float* linear2_svd1_bias_, float* norm2_weight_, float* norm2_bias_, int transformer_svd_dims_)
{
    ...
    linear1_svd0_weight = linear1_svd0_weight_;
    linear1_svd1_weight = linear1_svd1_weight_;
    linear1_svd1_bias = linear1_svd1_bias_;
    linear2_svd0_weight = linear2_svd0_weight_;
    linear2_svd1_weight = linear2_svd1_weight_;
    linear2_svd1_bias = linear2_svd1_bias_;
...
int Transformer_SVD::load_model(FILE* &fp)
{
    ...
    linear1_svd0_weight = new float[d_model * transformer_svd_dims];
    linear1_svd1_weight = new float[transformer_svd_dims * dim_feedforward];
    linear1_svd1_bias = new float[dim_feedforward];
    linear2_svd0_weight = new float[dim_feedforward * transformer_svd_dims];
    linear2_svd1_weight = new float[transformer_svd_dims * d_model];
    linear2_svd1_bias = new float[d_model];
    ...
    fread(linear1_svd0_weight, sizeof(float), d_model * transformer_svd_dims, fp);
    fread(linear1_svd1_weight, sizeof(float), transformer_svd_dims * dim_feedforward, fp);
    fread(linear1_svd1_bias, sizeof(float), dim_feedforward, fp);
    fread(linear2_svd0_weight, sizeof(float), dim_feedforward * transformer_svd_dims, fp);
    fread(linear2_svd1_weight, sizeof(float), transformer_svd_dims * d_model, fp);
    fread(linear2_svd1_bias, sizeof(float), d_model, fp);
    ...
    transpose(linear1_svd0_weight, transformer_svd_dims, d_model);
    transpose(linear1_svd1_weight, dim_feedforward, transformer_svd_dims);
    transpose(linear2_svd0_weight, transformer_svd_dims, dim_feedforward);
    transpose(linear2_svd1_weight, d_model, transformer_svd_dims);
...
int Transformer_SVD::forward(float* src, int bs, int in_c, float* dest, float* mask)
{
    ...
    //linear1_svd
    float* linear1_svd0_output = new float[bs * transformer_svd_dims]();
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, bs, transformer_svd_dims, d_model, 1.0, self_attn_output, d_model, linear1_svd0_weight, transformer_svd_dims, 1.0, linear1_svd0_output, transformer_svd_dims);

    float* linear1_svd1_output = new float[bs * dim_feedforward]();
    for (int i = 0; i < bs; i++) {
        memcpy(linear1_svd1_output + i * dim_feedforward, linear1_svd1_bias, dim_feedforward * sizeof(float));
    }
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, bs, dim_feedforward, transformer_svd_dims, 1.0, linear1_svd0_output, transformer_svd_dims, linear1_svd1_weight, dim_feedforward, 1.0, linear1_svd1_output, dim_feedforward);


    // relu
    for (int i = 0; i < bs * dim_feedforward; i++) {
        linear1_svd1_output[i] = linear1_svd1_output[i] > 0 ? linear1_svd1_output[i] : 0;
    }

    //linear2_svd
    float* linear2_svd0_output = new float[bs * transformer_svd_dims]();
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, bs, transformer_svd_dims, dim_feedforward, 1.0, linear1_svd1_output, dim_feedforward, linear2_svd0_weight, transformer_svd_dims, 1.0, linear2_svd0_output, transformer_svd_dims);
    for (int i = 0; i < bs; i++) {
        memcpy(dest + i * d_model, linear2_svd1_bias, d_model * sizeof(float));
    }
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, bs, d_model, transformer_svd_dims, 1.0, linear2_svd0_output, transformer_svd_dims, linear2_svd1_weight, d_model, 1.0, dest, d_model);
    ...
    delete[] linear1_svd0_output;
    delete[] linear1_svd1_output;
    delete[] linear2_svd0_output;
    delete[] src_pos;
...
int Transformer_SVD::matrix_split(float *src, float *dest, int height, int width, int start, int len) {
...
int Transformer_SVD::matrix_split_transpose(float *src, float *dest, int height, int width, int start, int len) {
...
int Transformer_SVD::transpose(float *src, int height, int width) {
    // convert A : a*b -> b*a
...
int Transformer_SVD::softmax(float* src, int height, int width){
...
int Transformer_SVD::shortcut(float *src, float *dest, int height, int width) {
    for(int i=0; i<height; i++){
        for(int j=0; j<width; j++){
...
int Transformer_SVD::layer_norm(float *src, int bs, int in_c, float* gamma_data, float* beta_data) {
...
int Transformer_SVD::position_embeding(float* src, int height, int width, float* mask) {
